/*
// $Id: //open/util/resgen/src/org/eigenbase/xom/MetaGenerator.java#4 $
...
/**
 * <code>MetaGenerator</code> is a utility class which reads a XOM Meta
...
//                          + "." + name + " that does not exist.");
            ...
            throws XOMException
        {
            ...
            if(superInfo == null)
                out.println("org.eigenbase.xom.ElementDef");
            else
            ...
            if(allContent.length == 0 && allAttributes.length == 0 &&
               !isAny && !isCData &&
                ...
                out.println("\t\t\t\tthrow new org.eigenbase.xom.XOMException("
                            + "\"In element '\" + getName() + \"': \" + _ex.getMessage());");
                out.println("\t\t\t}");
            }
        ...
        throws XOMException
    {
        ...
        out.println("<!--");
        out.println("     This dtd file was automatically generated by"
                  + " resgen (http:/" + "/eigenbase.sourceforge.net/resgen/) from "
        ...
        throws XOMException
    {
        ...
        out.println("/" + "/ by resgen (http:/" + "/eigenbase.sourceforge.net/resgen/)");
     ...
     * name so that it can be prepended with "get" or "set" to form a JavaBeans
     * getter/setter method name.
     */
    private String getBeanName(String name) {
        name = getBeanPropertyName(name);
    ...
    /**
     * Maps the given property name to some other name so that the setters and
     * getters don't collide with setters and getters defined by XOM.
     * 
     * @param name
     * @return
     */
        ...
        throws XOMException
    {
        ...
        out.println("\t\tpublic " + attr.type + " get" + getBeanName(attr.name) + "() {" );
        out.println("\t\t\treturn " + getDeclaredName(attr.name) + ";");
        out.println("\t\t}");

        out.println("\t\tpublic void set" + getBeanName(attr.name) + "(" + attr.type + " val) {" );
        out.println("\t\t\t" + attr.type + " oldval = this." + getDeclaredName(attr.name) + ";");
        out.println("\t\t\tthis." + getDeclaredName(attr.name) + " = val;");
        out.println("\t\t\t_pcs.firePropertyChange(\"" + getBeanPropertyName(attr.name) + "\", oldval, val);");
        out.println("\t\t}");
        out.println();
    }
